	...
	<div class="dynamic">
		...
		<div class="wrappers" ref="wrapper">
			<div>
				...
				<div class="pic-bg">
					<div class="pic-wrap">
						<img v-lazy="urlWechar" />
					</div>
					<div class="text-wrap">
						<div class="text-title">
							{{estateName}}
						</div>
						<div class="text-more" @click="goEstateDetail" style="display: flex;align-items: center;line-height:0rem;padding-top: .08rem;">
							<span>查看详情</span><span class="iconfont"></span>
						</div>
					</div>
				</div>
				<div class="content">
					<div class="content-item" v-for="(o, index) in dataNewsList" :key="index">
						<div class="content-item-header">
							<span class="date">{{o.createTime}}</span>
							<!-- <span class="tag">最新</span> -->
							<span class="more" @click="goDetail(o)" style="display: flex;align-items: center;line-height:0rem;padding-top: .08rem;"><span style>查看详情</span><span class="iconfont"></span></span>
						</div>
	...
	export default {
	  ...
	  beforeRouteLeave(to, from, next) {
		...
		methods: {
			...
			goEstateDetail() {
				this.$router.push('/mainPage/buildDetail?estateId=' + this.$route.query.estateId)
			},
			_initScroll() {
				if(!this.scroll) {
			...
			handleGetMoreEstateNewList() {
				this.Axios.post('/app/3.0/building_front/moreEstateNewList', {
					estateId: this.$route.query.estateId
				}).then((res) => {
					if(res.code === 0) {
						let data = res.data;
						this.estateName = data.estateName;
						this.urlWechar = data.urlWechar;
						this.dataNewsList = data.dataNewsList;
					}
				})
			},
			goDetail(item) {
		...
		created() {
			this.handleGetMoreEstateNewList()
		},
	}
</script>
